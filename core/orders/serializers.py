...
class OrderSerializer(ModelSerializer):
    responsible_person_data = UserSerializer(
        source="responsible_person", read_only=True
    )
...
class OrderItemSerializer(ModelSerializer):
    ...
    combo = PrimaryKeyRelatedField(
        queryset=Combo.objects.all(), required=False, allow_null=True
    )
    final_cup = PrimaryKeyRelatedField(
        queryset=FinalCup.objects.all(), required=False, allow_null=True
    )
    custom_cup = PrimaryKeyRelatedField(
        queryset=CustomCup.objects.all(), required=False, allow_null=True
    )
